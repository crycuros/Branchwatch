...
import {
  Search, CheckCircle2, Download, Trash2, Box,
  Archive, GitMerge, Tag, GitPullRequest, Settings2,
  Code2, ShieldAlert, Globe, HardDrive, Terminal, GitBranch,
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        ...
        <div className="flex items-center gap-1 bg-neutral-100 dark:bg-neutral-800/80 p-1 rounded-xl text-xs font-medium text-neutral-600 dark:text-neutral-400 self-start sm:self-auto border border-neutral-200 dark:border-neutral-700 overflow-x-auto max-w-full">
          {(['all', 'git', 'automation', 'testing', 'security', 'devops', 'custom'] as const).map((cat) => (
            <button
              key={cat}
            ...
            >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700 uppercase">
                          {plugin.runtime || 'shell'}
                        </span>
                      </div>
                      <div className="text-[10px] font-mono text-neutral-400">
                        v{plugin.version} · by @{plugin.author.login || plugin.author.name}
                      </div>
                    </div>
                  </div>
                ...
                {plugin.permissions && (
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {plugin.permissions.shell && (
                      <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                        <Terminal className="w-2.5 h-2.5" /> Shell
                      </span>
                    )}
                    {plugin.permissions.filesystem && (
                      <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                        <HardDrive className="w-2.5 h-2.5" /> FS: {plugin.permissions.filesystem}
                      </span>
                    )}
                    {plugin.permissions.network && (
                      <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20">
                        <Globe className="w-2.5 h-2.5" /> Network
                      </span>
                    )}
                    {plugin.permissions.git && (
                      <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-neutral-100 dark:border-neutral-800/60">
                <div className="text-[10px] text-neutral-400 font-mono">
                  {plugin.configSchema?.length || 0} parameter{plugin.configSchema?.length === 1 ? '' : 's'}
                </div>
      ...
      {filteredPlugins.length === 0 && (
        <div className="text-center py-12 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/40 text-neutral-400 text-xs">
          No plugins found matching &ldquo;{searchQuery}&rdquo;. Use Dev Studio to author custom nodes for your workflows.
        </div>
      )}
    </div>
  );
